...
global right_hang = {'right_s0':loose_position[11], 'right_s1':loose_position[12], 'right_e0':loose_position[9], 'right_e1':loose_position[10],
...
class Magic_Cube(object): 
    ...
    def Camera_Control(camera,command):
        assert (command =='open' or command=='close'), 'only two types of command  are allowed.'
        if command =='open':
            camera.open()
            print(camera.name()+' is opened.')
        else:
            camera.close()
            print(camera.name()+' is closed.')
